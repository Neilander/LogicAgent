"""节点图构建：在 stage0 基础上同时调用 food + water 识别核。"""

import numpy as np

from .recognizers.food import recognize_food
from .recognizers.water import recognize_water
...
_KIND_LABELS_CN = {"plant": "果实", "cow": "牛", "water": "水"}
...
def build_graph(semantic: np.ndarray, self_state: dict, view_radius: int = 4) -> Graph:
    ...
    water_hits = recognize_water(semantic, self_state["world_pos"], view_radius=view_radius)

    # 统一格式：[(dx, dy, raw_kind, abstract_kind)]
    items: list[tuple[int, int, str, str]] = []
    for dx, dy, raw in food_hits:
        items.append((dx, dy, raw, "food"))
    for dx, dy in water_hits:
        items.append((dx, dy, "water", "water"))
...
def graph_to_text(graph: Graph) -> str:
    ...
    if not graph.nodes:
        lines.append("  （空——视野内未发现 food/water）")
