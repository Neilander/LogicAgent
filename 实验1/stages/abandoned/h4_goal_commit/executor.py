...
def execute(
    ...
    elif plan["action"] == "flee_from":
        threat_node = graph.by_id(plan["threat"])
        if threat_node is None:
            return ["noop"]
        return _atomic_for_flee(threat_node, semantic, player_pos, rng)
    elif plan["action"] == "explore":
        return _atomic_for_explore(semantic, player_pos, rng)
    return ["noop"]


def _atomic_for_flee(threat_node: Node, semantic: np.ndarray, player_pos, rng: random.Random) -> list[str]:
    """逃离 threat：选远离威胁方向的 walkable 动作。"""
    tdx, tdy = threat_node._grid_pos
    # 期望的 escape direction：(- tdx, - tdy)
    # 找朝这个方向最接近的 walkable 动作
    candidates: list[tuple[str, int]] = []
    for action, (ddx, ddy) in _DIR_TO_DELTA.items():
        tile_id = _tile_after_move(semantic, action, player_pos)
        if tile_id not in WALKABLE_TILE_IDS:
            continue
        # 与逃跑方向的点积：越大越好
        score = ddx * (-tdx) + ddy * (-tdy)
        candidates.append((action, score))
    if not candidates:
        return ["noop"]
    # 按 score 降序，最 escape 的方向先
